  ...
  public class SRWindow : EditorWindow {
...
#if PSR_FULL
    [MenuItem ("Window/Search + Replace %#f")]
#else
    [MenuItem ("Window/Search %#f")]
#endif
    static void Init () 
    {
    ...
    void init()
    {
      ...
      if(!_initialized)
      {
        ...
        swapToggle.normal.background = EditorGUIUtility.FindTexture( "d_RotateTool" );
        swapToggle.hover.background = EditorGUIUtility.FindTexture( "d_RotateTool On" );
        swapToggle.onActive.background = EditorGUIUtility.FindTexture( "d_RotateTool" );
        swapToggle.margin = new RectOffset(0,4,0,0);
    ...
    void OnGUI () 
    {
      ...
      List<string> searchLabels = new List<string>();
      ...
      if(savedSearches.searches.Count > 0)
      {
        ...
        {
          searchLabels.Add(ss.name);
        }
        searchLabels.Add("-------");
        searchLabels.Add("Edit Searches");
      }
      searchLabels.Add("Save Search");
      int newSearchForIndex = EditorGUILayout.Popup( "Search:", searchForIndex, searchLabels.ToArray());
      ...
      GUI.enabled = currentSearch.CanSearch(searchOptions) && searchWhilePlaying; //NOTE: whether we can search while the application is playing is controlled by the search scope. 
      GUILayout.BeginHorizontal();
...
#if PSR_FULL
      GUI.enabled = currentSearch.CanSearchAndReplace(searchOptions) && searchWhilePlaying; //NOTE: whether we can search while the application is playing is controlled by the search scope.
      ...
      //This is supposed to stop stupidity with the selectable labels, but i don't see a difference. Tried at top and bottom.
      // if(Event.current.type == EventType.MouseDown)
      // {
      //   GUI.focusControl = "gooblegobblegooblegobble_oneofus!oneofus!";
    ...
    {
      bool newMode = GUILayout.Toggle(replaceMode, "Replace?", GUILayout.Width(75.0f));
#if PSR_FULL
      ...
      if(newMode)
      {
        EditorPrefs.SetInt(Keys.prefSearchType, 0);
        //Checked! Let them know that we have a lot more functionality in the full
        // version.
        if(EditorUtility.DisplayDialog("Unlock Full Version?", "Add the ability to search AND replace, choose your search location, and more!", "Open in Asset Store", "Close"))
        {
          Application.OpenURL(fullVersionURL);
        } 
      }
    ...
    static UnityEngine.Object findObject(string name)
    {
